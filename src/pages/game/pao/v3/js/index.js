...
NewWall.prototype.changeContent = function (content) {
  ...
  var temp = str.match(/得到(\d+)金币/);
  if(temp){
    str = str.replace(temp[0], "得到" + changeMoney(Number(temp[1])) + "元");
  }
...
function setIndexRankMe(data) {
  ...
  if (data.rank) {
    ...
    $meItem.find('.item-amount').html(changeMoney(data.amount) + "元")
        ...
        <div class="item-right-container">
          ...
          <div class="item-right">
            ...
            <div class="item-amount-wrap">
              ...
              <div class="item-amount">${changeMoney(item.amount)}元</div>
            </div>
          </div>
        </div>
      </div>`;
...
function setPlayerInfo() {
  ...
  $playInfo.find('.amount').html(changeMoney(CFG.amount) + "元")
