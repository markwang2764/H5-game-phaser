...
Shunt.prototype = {
    init: function(){
        ...
        $.get("/common/getGameGuideInfo", param, (data)=>{
            ...
            for(var i = 0; i < data.data.length; i++){
                embed.append(data.data[i].embed);
            }
            ...
            $(".other-games-item").click(function(){
                ...
                if(n === 0){
                    //点击埋点
                    embed.embedClick(embed.data.img_guide_first_click);
                }
                else{
                    //点击埋点
                    embed.embedClick(embed.data.img_guide_second_click);
                }
                setTimeout(()=>{
    ...
    show: function(){
        if(this.data){
            //曝光埋点
            embed.embedExport(embed.data.img_guide_first_exposure);
            embed.embedExport(embed.data.img_guide_second_exposure);
