...
(function () {
	var OverPage = function () {
		...
		// this.goldText.align = "center";
		// this.killText.align = "center";
		this.goldText.pivot.setTo(0.5,0.5);
		this.killText.pivot.setTo(0.5, 0.5);
	...
	p.update = function (dlt) {
		...
		this.updatePosition();
		this.updateNoGrowText();
	}

	p.updatePosition = function(){
		if(this.willPosition){
			var max = 200;
			if(this.killText.width > max){
				this.killText.scale.setTo(max * 2 / this.killText.width);
			}
			this.killText.y = 565;
			
			if(this.goldText.width > max){
				this.goldText.scale.setTo(max * 2 / this.goldText.width);
			}
			this.goldText.y = 432;
		}
	}
	...
	p.updateText = function (dlt) {
		...
		if(this.goldCounter >= this.gold_max || this.killCounter >= this.kill_max) {
			this.goldText.setText(this.gold_max);
			this.killText.setText(this.kill_max);
			this.runText = false;
		} else {
			this.goldText.setText(Math.floor(this.goldCounter));
		...
		var max = 200;
		if(this.goldText.width > max){
			this.goldText.scale.set(max * 2 /this.goldText.width);
		}
		this.goldText.y = -150 - this.goldText.height / 2;

		if(this.killText.width > max){
			this.killText.scale.set(max * 2 / this.killText.width);
		}
		this.killText.y = -24 - this.killText.height / 2;
	}
	...
	p.addNewKillRecord = function () {
		...
		tw.onComplete.add(function () {
			...
			tt.onComplete.add(function () {
				...
				twt2.to({x: 474, y: 556}, 200, null, true, 0, 0, false);//pppp
	...
	twt2.to({x: 474, y: 430}, 200, null, true, 0, 0, false);//pppp
	...
	p.addGrowText = function (board) {
		...
		this.goldText.anchor.setTo(0.5, 0.5);
		// 重要参数
		this.goldText.x = 99, this.goldText.y = -150;//pppp
		...
		this.killText.anchor.setTo(0.5, 0.5);
		// 重要参数
		this.killText.x = 99, this.killText.y = -45;//pppp

		this.killText.scale.setTo(0.1, 0.1);
	}

	p.dragPot = function(mc){
		mc.inputEnabled = true;
		mc.input.enableDrag();
		mc.events.onDragStop.add(function(mc){
	...
	p.showMonsterList = function (data) {
		var xianjin = this.game.add.sprite(-262, 60, 'xianjin');
		var hongbao = this.game.add.sprite(20, 60, 'hongbao');
		var again = this.game.add.sprite(-206, 390, 'again');
		var close = this.game.add.sprite(234, -400, 'close');

		setTimeout(()=>{
			var hand = this.game.add.sprite(-38, 4, 'hand');
			hand.animations.add('run');
			hand.animations.play('run', 6, true);
			this.bg_group.add(hand);
		}, 1500);

		this.bg_group.add(xianjin);
		this.bg_group.add(hongbao);
		this.bg_group.add(again);
		this.bg_group.add(close);
		
		// this.dragPot(hongbao);
		// this.dragPot(hand);
		// this.dragPot(again);
		// this.dragPot(close);

		// this.dragPot(this.goldText);
		// this.dragPot(this.killText);

		close.inputEnabled = true;
		xianjin.inputEnabled = true;
		hongbao.inputEnabled = true;
		again.inputEnabled = true;

		close.events.onInputDown.add(function(e){
			console.log("关闭");
			console.log(e);
			$(".index-page").show();
			$(".result-pop").hide();
		...
		xianjin.events.onInputDown.add(function(e){
			console.log("立即兑换");
			embed.numClick("6.2");
		...
		hongbao.events.onInputDown.add(function(e){
			console.log("有惊喜");
			embed.numClick("6.3");
			task.openType = 2;
            task.getReward();
		...
		again.events.onInputDown.add(function(e){
			embed.numClick("6.4");
			console.log("再玩一次");
			console.log(e);
			autoStart = false;
	...
	p.tweenOnGrowText = function () {
		...
		// t2.onUpdateCallback(function(){
		// 	this.updateNoGrowText();
		// }, this);
		
		this.willUpdateNoGrowText = true;
		setTimeout(()=>{
			this.willUpdateNoGrowText = false;
		}, 1800);
	}

	p.updateNoGrowText = function(){
		if(this.willUpdateNoGrowText){
			var max = 200;
			if(this.goldText.width > max){
				this.goldText.scale.setTo(max * 2 /this.goldText.width);
			}
			...
			// this.killText.y = -45 - this.killText.height / 2;
			this.killText.y = -40;
			console.log("scale = " + this.killText.scale.x, this.killText.y);
		}
	}
