...
var treasureBoxProbability = false;
var hitTreasureBox = false;
var bossBoxProbability = false;
var bossIsDie = false;
var boxData;
...
document.addEventListener('touchstart', function(e){
  var $div = $(e.target).parents(".renwu-list");
  if($div.length == 0){
    if($(".renwu-list").css("display") == "block"){
      $(".renwu-list").hide();
    }
  }
})
...
function onEnterGame() {
  if(CFG.firstLogin == 1 || CFG.freeAttackGet == 1){
    if(getParamUrl("tenter") && CFG.firstLogin == 1){
      startEnterGame();
    }
    else{
      popFirstGift(startEnterGame);
    }
  }
  else{
    startEnterGame();
...
function startEnterGame(){
  freeTimes = Number(CFG.freeAttack);
  if(game){
    game.gun.freeTimes = freeTimes;
    game.gun.changeTxt();
  }
...
function popFirstGift(callback){
  firstLogin = new FirstLogin(callback);
  if(CFG.firstLogin == 1 || CFG.freeAttackGet == 1){
    
    var num = Number(CFG.rewardAmount);
    $(".new-song-tip1 .ss").html(`X${CFG.freeAttack}次免费射击`);
    $(".new-song-tip2 .ss").html(`X${num}金币（￥${changeMoney(num)}）`);
    
    $(".pao-num").hide(); 

    if(CFG.firstLogin != 1){
      $(".new-song-tip2, .new-song-img2, .fire2").remove();
    }
    if(CFG.freeAttackGet == 0){
      $(".new-song-tip1, .new-song-img1, .fire1").remove();
    }
    else if(CFG.freeAttackGet == 1){
      $(".pao-mianfei").html("免费<br>X" + freeTimes).show();
    }
...
window.onload = function () {
  if(getParamUrl("contextToken")){
    var url = `${CFG.ytHost}?user_id=${CFG.userId}&game_id=${getParamUrl("id")}&share_way=${getParamUrl("share_way")}&contextToken=${getParamUrl("contextToken")}&deep=${getParamUrl("deep")}&shareTextId=${getParamUrl("shareTextId")}&needCharge=0&type=1`;
    $.get(url, function(){
  ...
  embed.init(function (suc) {
    ...
    store = new Store(function () {
      ...
      if(autoStart){
        ...
        addEvent();
        inRank = false;
        // $(".start-btn-rect,.new-tell,.title-name,.renwu-box,.renwu-title,.renwu-chaju,.room-gesture").hide();
        autoJumpGame();
      }
    })
...
function autoJumpGame(){
  var storestr = localStorage.getItem("pao_store");
  if(storestr){
    var newStore = JSON.parse(storestr);
    store.rankData = newStore.rankData;
    store.amount = newStore.amount;
    game.popResult(localStorage.getItem("pao_hasBox") == 1);
  }
  else{
    $(".cutdown-mask").css("display", "flex");
    setTimeout(() => {
      onEnterGame();
      $(".cutdown-mask").hide();
    }, 4000);
...
function addIndexEvent() {
  ...
  $(".btn-rank2,.btn-rule2").hide();

  $(".index-size-bg").click(function(){
    embed.singleClk(Embed.TYPE_16);
    onEnterGame();
  })

  $('.index-page .new-startbtn').on('click', function () {
    embed.singleClk(Embed.TYPE_1);
    $(".room-gesture,.title-name,.start-btn-rect,.renwu-box,.renwu-title").hide();
    setTimeout(()=>{
      $(".room-gesture,.title-name,.start-btn-rect,.renwu-box,.renwu-title").show();
    }, 4000);
    onEnterGame();
  });
